  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        {currentUserId === groupDetails?.adminId && (
          <TouchableOpacity style={styles.addMembersButton} onPress={() => setShowAddMemberPopup(true)}>
          ...
          {groupUsers.map((user) => (
            <View key={user.userId} style={styles.memberCard}>
              ...
              <View style={styles.avatarContainer}>
                {user.photoUrl ? (
                  <Image
                    source={{ uri: `https://YOUR_S3_OR_CDN_PATH/${user.photoUrl}` }}
              ...
              <ScrollView horizontal contentContainerStyle={{ flexWrap: 'wrap', marginVertical: 4 }}>
                {memberEmails.map((email) => (
                  <View key={email} style={styles.emailChip}>
                    <Text>{email}</Text>
                    <TouchableOpacity onPress={() => handleRemoveEmail(email)} style={{ marginLeft: 8 }}>
